...
def get_number_of_active_issues_by_column(column):
    print(f"######### Nombre de tâches actives pour une colonne donnée: {column} #########")
    ...
    for items in project_items:
        ...
        if column_name.lower() == column.lower():
            ...
            if "closed" in items["fieldValues"]["nodes"][0]["item"]["content"]:
                issues_count += 1
                is_closed = items["fieldValues"]["nodes"][0]["item"]["content"]["closed"]
                if is_closed:
            ...
            if items["fieldValues"]["nodes"][0]["item"]["content"]:
                issue_info = {
                    ...
                    "number": items["fieldValues"]["nodes"][0]["item"]["content"]["number"],
                    "title": items["fieldValues"]["nodes"][0]["item"]["content"]["title"],
                    "description": items["fieldValues"]["nodes"][0]["item"]["content"]["bodyText"],
                    "createdAt": items["fieldValues"]["nodes"][0]["item"]["content"]["createdAt"],
                    "closedAt": items["fieldValues"]["nodes"][0]["item"]["content"]["closedAt"] if items["fieldValues"]["nodes"][0]["item"]["content"]["closedAt"] else None
                }

                issue_list.append(issue_info)

    result = (f"Dans colonne: {column}\n"
