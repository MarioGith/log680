...
def get_issue_lead_time_within_period(start_date, end_date):
    print(f"######### Temps (lead time) pour les tâches terminées dans une période donnée ({start_date} - {end_date})#########")
    ...
    for issue_node in issues:
        ...
        if start_date is not None and end_date is not None and start_date < datetime.strptime(closed_at, "%Y-%m-%dT%H:%M:%SZ") < end_date:
            closed_issue_count += 1
            # Time between the issue's creation and closure
            time = datetime.strptime(closed_at, "%Y-%m-%dT%H:%M:%SZ") - datetime.strptime(created_at,
                                                                                          "%Y-%m-%dT%H:%M:%SZ")
            temp += time
    ...
    average = (temp - datetime.now())/closed_issue_count if closed_issue_count != 0 else 0
    print(f"Le temps moyen est de {average}")
    json_result = None
    if average != 0 :
        json_result = {
            "average_time": average.total_seconds() if average.total_seconds() == 0 else average.total_seconds(),
